      ...
      <div className="content-shell">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {codingProfiles.map(({ id, src, title, rating, description, link }) => (
            <AnimatedCard key={id} className="p-6">
              <div className="flex items-start justify-between gap-4">
                <img src={src} alt={title} className="h-14 w-14 rounded-2xl bg-white object-contain p-2" />
                <a
                  href={link}
                  target="_blank"
                  rel="noopener noreferrer"
                  aria-label={`Open ${title}`}
                  className="flex h-10 w-10 items-center justify-center rounded-full border border-white/[0.12] bg-white/[0.08] text-white transition hover:bg-white/[0.14]"
                >
                  <FiExternalLink />
                </a>
              </div>
              <h3 className="mt-6 text-2xl font-black text-white">{title}</h3>
              <p className="mt-3 text-sm font-bold text-cyan-100">{rating}</p>
